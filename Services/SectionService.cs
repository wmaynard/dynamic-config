...
public class SectionService : PlatformMongoService<Section>
{
  ...
  {
    List<string> names = _collection.Find(_ => true)
                                    .Project(Builders<Section>.Projection.Expression(section => section.Name)).ToList();
  ...
  public override IEnumerable<Section> List() => _collection.Find(_ => true)
                                                            .Project(Builders<Section>.Projection.Expression(section =>
                                                                                                               new
                                                                                                               Section(section
                                                                                                                         .Id)
                                                                                                               {
  ...
  public string GenerateAdminToken(string serviceName) => PlatformEnvironment.IsLocal
                                                            ...
                                                            : _apiService
                                                              ...
                                                              .SetPayload(new RumbleJson
                                                                          {
                                                                            ...
                                                                            {
                                                                              "screenname",
                                                                              $"{serviceName} ({PlatformEnvironment.Deployment})"
                                                                            },
                                                                            {"discriminator", 10_000},
                                                                            {
                                                                              "origin",
                                                                              $"{PlatformEnvironment.Name} ({PlatformEnvironment.Deployment})"
                                                                            },
                                                                            {
                                                                              "email",
                                                              ...
                                                              .OnSuccess(_ => Log.Local(Owner.Will,
                                                                                        "Admin token successfully generated."))
                                                              .OnFailure(response =>
                                                                           Log.Error(Owner.Will,
                                                                                     "Admin token failed to generate.",
                                                                                     data: new
                                                                                           {
                                                                                             Response = response
                                                                                           }))
